# DOSSIÊ — JANE QUEIROZ · JQL SEGUROS

**Documento vivo.** Atualizar aqui; rodadas de análise viram arquivos novos.
Última consolidação: **14/09/2026** · Fontes na seção 14.

> ✅ **Ciclo 2 contratado.** `EXP-2026-JQL-AS-02`, assinado por ela em **11/09**, R$1.850,
> vigência até **11/10**. Termos, mudanças de escopo e datas críticas em
> [CONTRATO-2026-09-11](CONTRATO-2026-09-11.md). A [estratégia](ESTRATEGIA-PERFIL-JANE.md)
> foi reescrita na chave **JQL Seguros como marca, Jane como a voz** — o §9 deste dossiê
> descreve a tese do ciclo 1 e está superado nesse ponto.
> Status atual: [360 de 14/09](360-JANE-2026-09-14.md) · [cronograma](CRONOGRAMA-2026-09.md).

> 🔴 **O ciclo 1 fechou hoje, 24/08, e fechou mal.** 4 posts publicados contra 12
> planejados · 10 dias sem publicação · 9 reclamações de qualidade · aprovação travada
> 5 dias · saída do social media e do tráfego em 20/08 · e a cliente perguntando em
> 22/08 se *"devido estar no primeiro mês promocional este cronograma não existe"*.
> Leitura completa em §15. Evidência em [ANEXOS-WHATSAPP-2026-08-24](ANEXOS-WHATSAPP-2026-08-24.md).

---

## 1. FICHA

| Campo | Valor |
|---|---|
| **Cliente (pessoa)** | Jane Queiroz — a marca é ela, não a empresa |
| **Empresa** | JQL Seguros (corretora) |
| **Nicho** | Seguros · Benefício Saúde (plano de saúde empresarial) |
| **Base** | Hortolândia/SP · escritório. Intenção declarada de mudar a sede para Alphaville |
| **Praças de atuação** | São Paulo (maioria da carteira), Rio de Janeiro, Belo Horizonte, Tocantins, Paraty |
| **Habilitação** | SUSEP plena |
| **Mercado desde** | Início de 2012 · corretora fundada em 2014 · **âncora de comunicação acordada: 2012** |
| **Perfil foco** | `@JaneJQL Saúde Seguros` (o Instagram que os clientes dela seguem) |
| **Perfil fora de escopo agora** | `@JQL Seguros` (institucional) — decisão registrada no doc de onboarding |
| **Entrada na Expansion** | Grupo criado por Kauã Catini em **24/07/2026** |
| **Onboarding** | 25/07/2026, 13h30, 56 min — [Fathom](https://fathom.video/calls/760724003) |
| **Modelo contratado** | **Plano teste — primeiro mês Expansion** |
| **Valor** | **R$1.300** (assessoria) **+ 2 diárias de videomaker × R$350** = **R$2.000** |
| **Canal oficial** | Grupo WhatsApp — renomeado em 03/08 para **"JANE SEGUROS and EXPANSION ®️"** (era "JQL SEGUROS…"; a decisão de marca saiu do papel) |
| **Primeiro post** | 29/07/2026 23:03 — [Reels](https://www.instagram.com/reel/DbZkF_EMyzG/), gravação na Agrotools |
| **Drive** | [Pasta da Jane](https://drive.google.com/drive/folders/1QQMwLfPDFa4H_sZHTjkMVDdUiAae_R5L) |

---

## 2. LINHA DO TEMPO

| Data | Evento |
|---|---|
| **27/05/2026** | Primeiro contato registrado: convite para o **Encontro de Negócios** (28–29/05, Plataforma Arena, Barueri). A origem do relacionamento é evento presencial, não anúncio. |
| **24/07 15:58** | Kauã Catini cria o grupo `JQL SEGUROS \| EXPANSION ASSESSORIA` → data prática do fechamento |
| **25/07 12:47** | Jane pede para remarcar o onboarding (vazamento na casa dela); ela mesma reabre 15 min depois: *"Vamos fazer agora"* |
| **25/07 13:30** | **Onboarding** por Google Meet — 56 min |
| **25/07 15:20** | Grupo renomeado para `JQL SEGUROS and EXPANSION ®️`; Adryel Silva (editor), Bernardo e mais 2 adicionados |
| **25/07 15:22** | Nicolas: *"Jane, tráfego pago vai fazer?"* → **sem resposta em texto até hoje** |
| **27/07 09:03** | Nicolas pede acesso da BM de anúncios + Instagram (login e senha) |
| **27/07 09:25** | Enviada a senha do Facebook **em texto puro no WhatsApp** (via Sabrina) → ver §12 |
| **27/07 15:01** | *"Vamos manter a comunicação por aqui"* — grupo vira canal oficial. Jane concorda. |
| **28/07 16:44** | **Jane pede o resumo do que terá no primeiro mês e quais redes.** Nicolas: *"te envio hoje um resumo"*. Jane: *"Não tem urgência."* |
| **28/07 22:51** | Jane passa endereço e logística da gravação de 29/07 |
| **29/07 09:30** | **Gravação 1** — Agrotools, Rua Hungria 888, 9º andar cj 92, São Paulo. Campanha de vacinação: **25 pessoas, 30 doses**. Filmmaker Jeferson Daniel chega atrasado; Jane cobra às 09:43. |
| **29/07 14:00** | **Gravação 2** — Rua Paraibuna 811, 12º andar, São José dos Campos *(mesma diária do dia 29)* |
| **29/07 15:15** | Pasta `Brutos › Campanha 29-07` criada no Drive (Fotos, Takes, criativos) |
| **29/07 19:42** | ✅ **Roteiros entregues** para aprovação ([doc](https://docs.google.com/document/d/1XUKwKTlqTP_zhgLp4-so1kMDl4Rt46-DWy6OgDBzHJo/edit)) |
| **29/07 19:52** | Jane solicita **autorização formal de imagem à Agrotools** por iniciativa própria |
| **29/07 22:18** | Primeiro vídeo editado entregue — ~12 h depois da gravação |
| **29/07 22:44** | ✅ **Primeira aprovação:** *"Amei"* · *"Gostei muito do vídeo"* |
| **29/07 23:03** | 📱 **Primeiro Reels publicado** — `instagram.com/reel/DbZkF_EMyzG/` |
| **29/07 23:07** | Ressalvas dela: *"achei a entrada meio escura"* · *"só tem estas [fotos]?"* |
| **30/07 08:41** | 🔴 **Jane aponta erro no vídeo já publicado:** nome da empresa errado, precisa ser Agrotools |
| **30/07 12:18** | **Gravação 3** — Hortolândia, escritório dela, com o filmmaker **Emmanuell** *(2ª e última diária contratada)* |
| **30/07 17:16** | 🔴 Jane cobra: a Agrotools foi **marcada, não posta como colaboradora** — *"aparece no feed da empresa"* |
| **30/07 17:17** | ⭐ **Segunda aprovação, e forte:** *"você vai amar"* · *"as fotos ficaram lindas, os vídeos ficaram lindos"* · *"ele não está pondo a arma na minha cabeça, eu estou falando mesmo"* |
| **30/07 17:43** | Collab corrigido; falta a Jane **aceitar pelo perfil da JQL** — sem confirmação de que aceitou |
| **30/07 18:07** | 💡 Jane propõe pauta nova: **bastidor / erros de gravação** da campanha de vacinação |
| **30/07 19:07** | Jane manda o **próprio banco de imagens** ([pasta](https://drive.google.com/drive/folders/1y72d2PsqTJqB5yEPYZCFVPF8sC1GT90y)) |
| **31/07 15:46** | Áudio da Jane — único contato do dia. Era só *"depois você me dá um alô aqui"* |
| **01/08 12:35** | Nicolas: *"não consegui ouvir ainda"* — **21 h de silêncio**. Reforça centralizar no grupo |
| **03/08 12:03** | Nicolas promete *"hoje"* o projeto completo e o cronograma — **2ª promessa do mesmo pedido de 28/07** |
| **03/08 12:03** | Grupo renomeado: `JQL SEGUROS…` → **`JANE SEGUROS and EXPANSION ®️`** |
| **03/08 14:56** | Jane pergunta sobre **competência do pagamento** |
| **03/08 17:32** | Nicolas define o ciclo: pagamento 24/07 · entrega desde 25/07 · **alinhamento em 24/08** |

---

## 3. A PESSOA — o que ela quer que apareça

Jane definiu a marca por identidade, não por métrica. A palavra que **ela mesma** trouxe e repetiu é **LEVEZA**.

**Deve aparecer**
- Mineira, de família humilde — *"estou aqui buscando um pedacinho do meu espaço, conquistei muita coisa, ainda desejo conquistar muito mais"*
- **Simplicidade sofisticada** — a ponte declarada entre as duas classes: *"me conectar tanto com pessoas de classes mais baixas quanto com as mais altas"*
- Elegância. Gosta de estar bem-arrumada, maquiada, "falar certinho"
- Esposa de um sírio (culturas diferentes — ela e o Nicolas concordaram que isso conecta), cristã, mãe
- O cotidiano real da empreendedora, na ordem em que ela mesma narrou: *"de manhã no médico, antes de ir para o evento; sai do evento, leva o menino no médico; sai do médico para atender cliente; volta, leva o menino no judô, volta, faz janta, arruma a casa, vai dormir, acorda cedo, vai na academia — essa é a Jane"*
- Cuida da saúde: pedala, academia
- Mulher empreendedora com chancela pública: convidada pela **Prefeitura de Hortolândia** para o encontro de Mulheres Empreendedoras da cidade (terça, 28/07). Novo evento previsto: **Grupo Eva, setembro** — possível palestra.

**Não pode aparecer**
- Piada, "engraçadinho" — *"não gosto muito de piadas"*
- Gírias
- Imagem "durona"
- Tom sério demais: *"eu não quero nada desse negócio maçante, pesado"*

**Referências que ela deu**
| Perfil | O que ela admira | Ressalva dela |
|---|---|---|
| `@convenioscomabruna` | Vende muito no orgânico, sem anúncio; fala dos produtos | *"eu queria um pouquinho diferente"* |
| **Lemo Corretora** (`lemo` com dois M) | Comunicação de **experiência do cliente** — é onde ela quer chegar | Contrataram a agência que atende a Boca Rosa (~R$100 mil, segundo ela) |
| **WEG Seguros** | Perfil humanizado — *"acho espetáculo"* | — |
| Pablo Marçal | Mestre em aproveitar o hype do momento | Citado como técnica, não como tom |

---

## 4. O NEGÓCIO

**Carro-chefe:** Benefício Saúde — **plano de saúde empresarial (coletivo empresarial)**, em três formatos:

| Formato | Descrição | Posição estratégica |
|---|---|---|
| Pequena empresa / familiar | Família contrata via CNPJ | Entra, mas não é o alvo |
| **30 a 99 vidas** | Empresa nem tão grande nem tão pequena | ✅ **Foco do funil** |
| 100+ vidas (corporativo/PJ) | Exige gestão de risco, mapeamento de saúde, palestras, entrega de vacinas | ⚠️ Não é o alvo |

**Por que 30–99 vidas e não 100+** (na palavra dela): acima de 100 vidas ela concorre com grandes corretoras de estrutura superior, **o ticket é menor** e a JQL não assume a operação sozinha — entra com corretora parceira de porte maior (Serra Corretora, Lemo Corretora).

> Isso é a chave do funil. O conteúdo tem que atrair **dono ou RH de empresa de 30 a 99 vidas** — não empresário genérico, não pessoa física.

**Outros produtos** (vende, mas não oferece ativamente — *"eu não vendo, eu não ofereço, mas quando o cliente tem interesse, eu fecho"*): seguro auto, seguro de vida, seguro saúde internacional, cascos marítimos (Paraty), posto de gasolina.

**Serviço de concierge — a rotina que virou o diferencial:**
- **Renovação:** avisa o cliente **60 dias antes**, informa o índice, negocia com a companhia, faz estudo de mercado
- **Boleto:** mensagem automática mensal (WhatsApp/e-mail), envia login (e senha, se autorizado), acompanha adimplência
- **Sinistro:** conduz o processo ponta a ponta e solicita reembolso
- **Gestão de risco / mapeamento de saúde / palestras / vacinas:** para as contas grandes

---

## 5. PÚBLICO-ALVO (o ICP da Jane)

| Dimensão | Definição |
|---|---|
| **Classe** | AAA |
| **Quem** | Empresários, donos de empresa **com funcionários**; nas contas empresariais, também o **RH decisor** |
| **Profissões** | Empreendedores, médicos, dentistas, advogados, artistas de TV, investidores. *Não CLT.* |
| **Idade** | 35+ · média real da carteira 44–58 |
| **Vida pessoal** | Esposa, dois filhos, tira férias com a família |
| **Comportamento** | Antenado nos processos da própria empresa; está nas redes **acompanhando** — não necessariamente com audiência (*"tenho cliente com 24 seguidores que é dono de transportadora que atende o Brasil inteiro"*); gosta de atendimento personalizado e de visita presencial |
| **Faturamento de referência** | R$20–50 mil/mês *(⚠️ ambíguo na call — pode ser faturamento do cliente ou ticket. Confirmar antes de usar em filtro de MQL)* |
| **🚫 Regra de exclusão declarada** | *"Eu nem quero gente que não tenha seguro — a pessoa que não tem seguro não tem consciência ainda do seguro."* |

**O que consomem** — usar como linguagem de qualificação no conteúdo:
- **Planos:** Bradesco linha top · SulAmérica linha executiva · Amil linha Black · Omint (o mais caro do Brasil)
- **Hospitais:** São Luiz Morumbi · Oswaldo Cruz · Samaritano Higienópolis · Sírio-Libanês · Einstein · Mater Dei (BH) · Rede D'Or (RJ)

**Evidência de que o tráfego atual está desqualificado** — ela já roda anúncio de Amil Black por conta própria:
> *"Está chegando algumas leads, mas idade... 59 anos, a pessoa quer pagar mil reais. A Amil Black, no mínimo, 5 mil reais num perfil assim."*

Isso é dado, não opinião: **o funil pago dela hoje traz o oposto do ICP.** Ver §11, pendência de tráfego.

---

## 6. AUTORIDADE — o que sustenta a marca

**Números** (declarados pela cliente, ⚠️ não auditados — confirmar antes de virar arte ou bio):
- **Desde 2012** no mercado de seguros · corretora desde 2014
- **+5.000 vidas protegidas** — ela diz *"acho que até mais que isso, na verdade"*
- **SUSEP plena**
- **Google: avaliações 5 estrelas** — pede avaliação a todos os clientes. *Ativo de prova pronto e ainda não coletado pela Expansion.*

**Carteira citável (empresas)**
| Cliente | O que é |
|---|---|
| **Agrotools** | Tecnologia para agronegócio; atende Google e Sadia; ~300 funcionários. **É a conta da gravação de 29/07.** |
| JBQ Global | Tecnologia |
| Delende | Tecnologia |
| Rede OK | — |
| Osas Tecnologia | Tecnologia |
| **FW Transportes** | Transportadora — o dono (Francisco) é amizade próxima; origem do case do curativo |
| **Medi** | Distribuidora de medicamentos — **o dono gravou depoimento em vídeo e a própria empresa repostou** |
| Atrasorb | Ex-empregadora dela, hoje cliente. Fornece Cal Sodada para a Marinha do Brasil e mergulho profissional (Petrobras). Ligada a Titan Portas — também cliente. |
| Cirúrgica Fernandes | Distribuidora hospitalar — conta que ela abriu como gerente comercial |

**Carteira citável (pessoas)**
- **Ana Vitória** (dupla) — corretora delas desde o começo da carreira em São Paulo
- **Thiago York** — chegou via Ana Vitória
- **Marcílio** — dono de ~4 empresas; *"até a faxineira dele tem Omint"*
- Um **youtuber de games** (>100 mil visualizações) — hoje ela atende ele, a mãe e a irmã
- Médicos e dentistas conceituados; famílias em Tocantins; uma palestrante mineira famosa no RJ (nome não lembrado na call)

**A história de origem** — vale um Reels inteiro: entrou na Atrasorb **como recepcionista**, virou **gerente comercial em 6 meses**, ficou 6 anos, vendeu Cal Sodada para a Marinha do Brasil e para a Petrobras, e existe **uma foto dela no topo de um submarino num folder da empresa**. Quebrou uma rixa histórica entre a família alemã dona da Atrasorb e a Cirúrgica Fernandes (judeus) mandando um e-mail por conta própria; o gerente homem foi enviado no lugar dela e o cliente recusou — *"eu não marquei com você, eu marquei com a Jane"*. Fecharam 50 kg de Cal Sodada. Pediu as contas logo depois para virar corretora.

---

## 7. OS CASES — a matéria-prima de conteúdo

São o ativo mais forte que ela tem. Todos saíram da própria boca dela na call.

**1 · Curativo a vácuo — R$10 mil recuperados (FW Transportes)**
Pai do Francisco, 95 anos, cirurgia no Einstein. A Amil cobriu o curativo a vácuo na internação, mas negou as três trocas seguintes em casa. O cliente pagou **R$10 mil** do bolso. A corretora acionou o concierge da Amil, conseguiu vaga no Oswaldo Cruz pelo convênio para as trocas restantes e pediu o reembolso dos R$10 mil.
> *"O cliente não fez nada, Nicolas. Não fez nada. A gente fez todo o contato."*

**2 · Remissão de R$6 por mês → R$55 mil + R$15 mil para a família**
Ela incluiu remissão num contrato sem o cliente saber que tinha. O funcionário faleceu; a família recebeu **R$55 mil mais R$15 mil** para o funeral. Custo da cláusula: **R$6/mês a mais**.
*⚠️ Confirmar os valores e a mecânica com a Jane antes de publicar — a narrativa saiu embolada na call.*

**3 · A carência de 6 meses do CNPJ**
Diretor demitiu três funcionários para recontratá-los como PJ e quis migrá-los no Bradesco na hora. Jane travou: precisa de contrato social ou requerimento; se for MEI, **6 meses de CNPJ aberto** e emissão de nota fiscal; natureza jurídica **213-5** (empresário individual). Evitou a perda de cobertura dos três.

**4 · O plano regional vendido para uma transportadora nacional**
Corretor anterior vendeu plano de cobertura regional para uma transportadora cujos motoristas rodam o Brasil inteiro. Fora de São Paulo, sem cobertura — e a empresa amarrada no contrato. É o case que sustenta o diferencial *"não ofereço o que eu quero, ofereço o que o cliente precisa"*.

**5 · O aplicativo errado — 300 vidas**
Empresa de 300 vidas com sinistralidade alta em pronto-socorro. Causa: os funcionários baixaram o **Bradesco Seguros** em vez do **Bradesco Saúde** — e o app errado não tem telemedicina. Ninguém usava telemedicina porque ninguém sabia que existia. Ela marcou reunião online com os funcionários e trouxe o Bradesco junto.
> *"Parece bobo, né, Nicolas? Mas não é bobo."*

**6 · O comentário que virou três clientes**
Dr. Elton (advogado, grande no Instagram) postou sobre portabilidade de carência de forma genérica. Ela comentou sugerindo o que faltava. **Aquele comentário gerou três clientes.** Um deles: implantou a apólice e depois fez a portabilidade da criança, que não seria aceita no ato da venda.
> Esse case é a prova de conceito da tática de reação. Deve virar formato recorrente, não post único.

---

## 8. DIFERENCIAIS — a lista de vendas

1. **Atendimento personalizado e acompanhamento pós-venda** (o que ela cita primeiro, sempre)
2. **Análise detalhada de mercado** — *"não ofereço o que eu quero, ofereço o que o cliente precisa"*
3. **Experiência** — sabe o que funciona e o que não funciona, evita a armadilha antes dela acontecer
4. **Concierge e equipe** — *"atendimento rápido, que se preocupa mesmo em ajudar o cliente ali na ponta"*
5. **Ética e respeito pelo tempo do cliente** — não força venda de quem não precisa
6. **Seguro saúde internacional**

A pergunta que ela mesma faz e que deveria organizar toda a comunicação:
> *"Quando você fecha plano de saúde, ele é tabelado, todo corretor tem o mesmo preço. O que faz você fechar comigo e não com outro?"*

Resposta dela: relacionamento e pessoa. **Por isso a marca é a Jane, não a JQL.**

---

## 9. ESTRATÉGIA ACORDADA

**Definição de sucesso — da cliente, literal:**
> *"Conseguir me ver ali na rede social. De fato, entregar o que realmente eu sou. Não ser só mais uma pessoa contando uma história — ver mesmo a minha essência."*
> *"Eu não falo de engajamento, eu não falo de gerar leads. Nesse primeiro momento é tentar me encontrar como profissional e poder repassar isso."*

⚠️ **O critério de aceite deste primeiro ciclo é subjetivo e pertence à cliente.** Não é CTR, não é lead, não é seguidor. É reconhecimento. Isso muda como a entrega tem que ser apresentada: **mostrar a ela, e pedir que ela se reconheça**, antes de mostrar número.

**Estratégia da Expansion (Nicolas, na call):** posicionar a **pessoa Jane Queiroz como centro de um ecossistema**. A audiência é construída em torno dela; a JQL Seguros e o que vier depois (podcast, YouTube, palestras) são alimentados por essa audiência.
> *"Se você se posicionar, já era — você vai construir audiência, as pessoas vão gostar tanto de você que, de cliente, quantas indicações vão sair dele."*

**Comercial** — plano teste do primeiro mês:

| Item | Valor |
|---|---|
| Assessoria | R$1.300 |
| 2 diárias de videomaker × R$350 | R$700 |
| **Total** | **R$2.000** |

**Ciclo** (definido por Nicolas em 03/08): pagamento em **24/07** → entrega começa em
**25/07** → **24/08 fecha os 30 dias e tem reunião de alinhamento.** É a data de
decisão de renovação.

As duas diárias contratadas **já foram consumidas**: 29/07 (Agrotools São Paulo de
manhã + São José dos Campos à tarde — mesma diária) e 30/07 (escritório dela, em
Hortolândia). Gravação nova neste ciclo é custo adicional. Ela já perguntou se pode
gravar com o próprio iPhone — vale ter uma resposta pronta, porque é a saída natural
quando a diária acaba.

⏳ **Lista de entregáveis totais do plano: pendente** (Nicolas vai enviar). Sem ela,
não dá para responder com precisão a pergunta que a cliente fez em 28/07 — e é ela que
fecha a pendência #1.

**Fases**
| Fase | Objetivo |
|---|---|
| **1 — agora** | Preparar o Instagram, contar a história da Jane, mostrar quem ela é, gerar conexão |
| **2** | Vendas orgânicas — *"o cliente se interessou: olha Jane, você trabalha com esse plano? consegue me ajudar?"* |
| **3 — futuro** | YouTube e podcast (proposta do Nicolas: tempo de tela longo gera confiança), parcerias, Hub de negócios da Tower |

**Redes:** Instagram é o foco. Ela quer também **TikTok, Facebook e Threads**.

**🚨 Gatilho de cancelamento — declarado por ela quando perguntada diretamente:**
> *"Não transmitir o que eu espero que transmita. A gente só consegue ver valor no que realmente a gente consegue se conectar."*

Não há segundo motivo. É esse.

---

## 10. TIME E CANAIS

**Lado JQL Seguros**
| Pessoa | Papel |
|---|---|
| **Jane Queiroz** | Decisora única de conteúdo e marca |
| **Amanda** | Assistente — tem a agenda e guarda as senhas. **Deve ser adicionada ao grupo** (combinado na call) |
| Gabi | Equipe — estava de férias em 25/07 |
| Sabrina | Quem repassou a senha do Facebook em 27/07 |
| **Aline** | Contato na **Agrotools** — participou da gravação de 30/07. Quase certamente a mesma Aline do caso do app do Bradesco na call (empresa de 300 vidas) |
| Consultoria financeira | Nova; a corretora está migrando do Simples para o Lucro Real |

**Lado Expansion**
| Pessoa | Papel |
|---|---|
| **Nicolas Nascimento** | Estratégia, relacionamento, aprovação |
| **Kauã Catini** | Comercial — agendou o onboarding e criou o grupo |
| **Dani** | Social media — subiu o primeiro Reels em 29/07 |
| Adryel Silva | Editor |
| **Matheus Felipi** | Tráfego — contato enviado à Jane em 27/07, junto com o pedido de acesso à BM |
| Bernardo + 2 | No grupo |
| **Jeferson Daniel** | Filmmaker — diária de 29/07 (Agrotools SP + SJC) |
| **Emmanuell Oliveira** | Filmmaker — diária de 30/07 (Hortolândia). **Elogiado nominalmente pela cliente** |

**Canal oficial:** grupo `JANE SEGUROS and EXPANSION ®️`. Combinado em 27/07, reforçado em 01/08 (*"pode centralizar todas as comunicações no grupo"*). Na prática **não funcionou**: o grupo tem 12 mensagens em 11 dias e toda a operação real — entrega, aprovação, correção, pagamento — acontece no privado com o Nicolas.

> ⚠️ Isso concentra o cliente numa pessoa só. Se o Nicolas sai de circulação, a conta para — e o histórico de decisão fica invisível para o time.

---

## 11. PENDÊNCIAS E RISCOS

Situação em **04/08/2026**, a **20 dias** da reunião que fecha o ciclo (24/08).

### 📦 Entregue em 04/08

| Entregável | Onde |
|---|---|
| **Plano de 30 dias** — posicionamento, perfil, grade e cronograma, em documento visual para a cliente | [`entregaveis/PLANO-30-DIAS-JANE.html`](entregaveis/PLANO-30-DIAS-JANE.html) |
| **Cronograma de agosto** — 12 posts sobre o material de 29 e 30/07 | [`CRONOGRAMA-2026-08.md`](CRONOGRAMA-2026-08.md) |
| **9 roteiros novos** (06–14), padrão TACO(H) | [Google Doc "Jane Queiroz"](https://docs.google.com/document/d/1CRbq0PYHba0ZUNGuZpRz8OSy2_keGNz3MqTtx_BFzow/edit) na pasta dela |

✅ **Enviado em 04/08 18:03 e aprovado por ela:** *"ótimo"* · *"ficou top"*.
Fecha a pendência que estava aberta desde 28/07.

### ✅ Fechadas desde a última rodada

| Pendência | Como fechou |
|---|---|
| **Roteiros para aprovação** | Entregues em 29/07 19:42, em Google Doc |
| **Autorização de uso de imagem da Agrotools** | A **própria Jane** pediu formalmente por escrito em 29/07. Guardar o documento. |
| **Definição do perfil / marca** | Resolvida na prática: grupo renomeado para `JANE SEGUROS…` em 03/08. Pessoa acima da empresa. |
| **Banco de imagens** | A Jane mandou o dela em 30/07 |

### 🔴 Abertas — vermelhas

| # | Pendência | Dono | Desde | Por quê |
|---|---|---|---|---|
| 1 | **Resumo / projeto / cronograma** | Nicolas | 28/07 | **7 dias e duas promessas de "hoje"** — 28/07 e 03/08. Ver abaixo. |
| 2 | **Lista de entregáveis do plano** | Nicolas | 29/07 | É o que falta para a #1 poder ser respondida com precisão |
| 3 | **Senha do Facebook em texto puro** | Nicolas | 27/07 | Continua sem troca. Ver §12 |
| 4 | **Erro de nome de empresa em vídeo publicado** | Expansion | 30/07 | Corrigido? Não há confirmação no chat. Ver abaixo. |
| 5 | **Dúvida de competência do pagamento** | Nicolas | 03/08 | Ela falou *"esses pagamentos"*, no plural, e *"para não ter problema"*. Ver abaixo. |
| 6 | **Convite de Collab da Agrotools foi aceito?** | Jane + Expansion | 30/07 | Ela disse *"vou ver se consigo liberar, eu te aviso"* e **nunca avisou**. Sem aceite, o Reels não entrou no feed da Agrotools — que era o objetivo. Ver abaixo. |

### 🟡 Abertas — amarelas

| # | Pendência | Dono | Desde |
|---|---|---|---|
| 6 | **Tráfego pago: decisão formal** — segue em aberto | Nicolas + Jane | 25/07 |
| 7 | **Acesso ao Instagram** — veio só a senha do Facebook | Jane / Amanda | 25/07 |
| 8 | **Falta material com a Jane em cena** — *"só tem estas?"*; as 2 diárias acabaram | Expansion | 29/07 |
| 9 | **Ela pode gravar com o próprio iPhone?** Pergunta sem resposta clara | Nicolas | 29/07 |
| 10 | **Iluminação** — *"achei a entrada meio escura"* | Filmmaker | 29/07 |
| 11 | **Depoimentos e prints do Google 5★** — prova pronta, parada | Expansion | — |
| 12 | **Números de autoridade não auditados** (+5.000 vidas, desde 2012) | Jane | — |
| 13 | **Amanda no grupo** | Jane | 25/07 |
| 14 | **Comunicação não migrou para o grupo** — ver §10 | Expansion | 27/07 |

---

### A leitura do ciclo até aqui

**A entrega funcionou, e a cliente disse isso duas vezes.** Gravação em 29/07 de manhã, vídeo editado às 22:18 do mesmo dia, aprovação às 22:44, publicado às 23:03 — cerca de 12 horas do bruto ao ar.

> **29/07 22:44** — *"Amei."* · *"Gostei muito do vídeo."*
>
> **30/07 17:17** — *"O Emmanuell está de parabéns. (…) Nossa, Nicolas, você vai amar. Ficou muito, muito legal. Tô muito feliz. Fiquei muito empolgada. As fotos ficaram lindas, os vídeos ficaram lindos. (…) Ele tá aqui do meu lado, **mas ele não está pondo a arma na minha cabeça, tá? Eu estou falando mesmo.**"*

A segunda é mais forte que a primeira, e ela própria antecipa e desarma a leitura de que seria elogio por educação. Dentro do critério declarado em §9 — *"conseguir me ver na rede social"* — **isso é o resultado.** É o ativo mais valioso do ciclo e é ele que deve abrir a conversa de 24/08.

**O que está minando isso é administrativo, não criativo.** Quatro coisas:

**1 · O resumo que não chega.** Pedido em 28/07, prometido para o mesmo dia. Prometido de novo em 03/08 — *"já te encaminho hoje todo o projeto e o que iremos fazer, e também o cronograma do perfil"*. Não chegou nas duas vezes. A cliente já tinha dito na call *"eu não sei o que está incluso"*, e respondeu *"não tem urgência"* quando a promessa furou. Cliente educado diz isso pouco antes de parar de perguntar.

O histórico da casa está em [`OPERACAO-REAL-EXPANSION-2026-07-26.md`](../../OPERACAO-REAL-EXPANSION-2026-07-26.md): o maior cliente quase cancelou **por ausência de relatório, não por qualidade de conteúdo**. Com um critério de aceite subjetivo (§9), o risco não é ela reclamar — é ela não conseguir dizer o que recebeu quando chegar 24/08.

**2 · O erro que a cliente pegou.** O primeiro Reels foi ao ar em 29/07 23:03 com o nome da empresa errado, e quem viu foi a Jane, 9h30 depois — num vídeo que marcava a Agrotools, o maior cliente dela. É o erro nº 1 da lista do [`POP-SOCIAL-MEDIA-v0.1.md`](../../POP-SOCIAL-MEDIA-v0.1.md). Ela avisou com um 💜 no fim, o que suaviza mas não anula: **a conferência antes de postar não aconteceu.** E não há no chat confirmação de que foi corrigido.

**3 · O silêncio de 21 horas.** Áudio dela em 31/07 15:46, resposta em 01/08 12:35 — e a resposta é *"não consegui ouvir ainda"*. O áudio tinha **6 segundos** e dizia só *"tudo bem? depois você me dá um alô aqui"*. Não era cobrança — o que torna a demora mais difícil de justificar, não menos. Foi o único contato de 31/07, 01 e 02/08, sem entrega registrada nesses três dias.

**4 · O Collab que talvez nunca tenha sido aceito.** Em 29/07 ela escreveu *"posta e marca a Agrotools, quem sabe eles aceitam o convite"*. **"Aceitam o convite" é Collab do Instagram, não marcação** — e o time só marcou. Ela conferiu no dia seguinte e explicou: *"quando você coloca como colaborador, aparece no feed da empresa"*. O Collab foi então adicionado, mas depende de aceite pelo perfil da JQL; ela disse *"vou ver se consigo liberar, eu te aviso"* e **não avisou**.

Se não foi aceito, o primeiro post da conta **nunca chegou ao feed do maior cliente dela** — que era o ponto todo. Verificar leva um minuto.

### A dúvida de pagamento

Em 03/08 ela perguntou por escrito e por áudio. O áudio é mais claro que o texto:

> *"Esses pagamentos que eu fiz para vocês, ele é o mês de 8 vigente, como que é? Depois você pode me explicar, por favor, **para não ter problema**?"*

**"Esses pagamentos", no plural.** A resposta de 17:32 falou apenas do pagamento de 24/07. Se houve um segundo em agosto, a pergunta segue aberta.

**Resolver antes de 24/08.** Dúvida de cobrança pendurada na véspera da renovação transforma "não renovo agora" numa decisão fácil — e o *"para não ter problema"* mostra que ela já está tratando isso como coisa a evitar.

### O ponto cego do tráfego

Ela já roda anúncio sozinha e o funil traz o oposto do ICP (§5). O contato do **Matheus Felipi (tráfego)** foi enviado a ela em 27/07, junto com o pedido de acesso à BM — ou seja, a operação **já começou a montar tráfego** enquanto a decisão formal segue em aberto desde 25/07. As pastas `Editados › TRÁFEGO` e `Brutos › Campanha 29-07 › criativos` existem no Drive desde julho.

A conversa certa não é "faz ou não faz". É: **a conta de anúncio que já está rodando está trazendo lead de R$1.000 para um produto de R$5.000** — e isso ou entra no escopo, ou é desligado.

---

## 12. CONTEÚDO SENSÍVEL — não publicar

- 🔐 **A senha do Facebook da Jane trafegou em texto puro no WhatsApp em 27/07 e está nos ZIPs exportados.** **Não replicada neste repositório.** Ação: trocar a senha, migrar para cofre (1Password/Bitwarden) e pedir acesso por **Meta Business Suite com convite por e-mail** — nunca por login e senha.
- 🔐 **CPF de terceiro em texto puro** (29/07 20:11, provavelmente liberação de acesso a prédio) e **telefone celular** de membro do time. **Não replicados aqui.** Dado pessoal de terceiro não circula em grupo de cliente — para acesso a prédio, mandar direto ao contato da portaria.
- 📄 **A autorização de imagem da Agrotools** foi solicitada pela Jane em 29/07. **Guardar o documento assinado** — é o que libera usar o material dessa gravação. Sem ele em mãos, o Reels de 29/07 está publicado sem lastro documental do lado da Expansion.
- **Caso "Dr. Pedro"** (médico, fraude apurada por operadora): citado na call. **Não usar em nenhum formato**, nem anonimizado — a cadeia de identificação é curta demais.
- **Nomes de clientes** só entram em conteúdo com **autorização por escrito**. Vale para Agrotools, FW, Medi, Ana Vitória, Thiago York e todos os demais.
- **Valores de sinistro e reembolso** ligados a pessoa identificável são dado de saúde. Nos cases (§7), despersonalizar: *"um cliente de 95 anos"*, não *"o pai do Francisco, da FW"* — a menos que o Francisco autorize.
- **Nomes de operadoras de saúde internacional**: ela disse na call que não pode citar. Respeitar.

---

## 13. ESTRUTURA DO DRIVE

```
Jane JQL Seguros/
├── Documentos/                         ← criada em 14/09; tudo que é documento mora aqui
│   ├── Contrato EXP-2026-JQL-AS-02 (assinado)   ← PDF Clicksign
│   ├── Contrato EXP-2026-JQL-AS-02 — prazos e obrigações
│   ├── Plano de 30 dias — Ciclo 1 (agosto)
│   ├── Jane Queiroz                    ← roteiros 06–14, padrão TACO(H)
│   ├── Jane SQL SEGUROS - ONBOARDING   ← doc bruto do onboarding (íntegra nos anexos)
│   ├── Cópia de MOD - BRIEFING DE CLIENTE  ← template em branco, NÃO preenchido
│   └── Cópia de ROTEIRO SEMANAL - MODELO   ← template em branco, NÃO preenchido
├── Brutos/
│   └── Campanha 29-07/ (Fotos ~100 JPGs · Takes · criativos)
├── Editados (Organico & tráfego)/
│   ├── ORGANICO/   (vazia em 29/07)
│   └── TRÁFEGO/    (vazia em 29/07)
└── Materiais (vídeos & fotos)/  (vazia em 29/07)
```

Dois templates estão lá em branco desde 26/07: o **briefing de cliente** e o **roteiro semanal**. O briefing pode ser preenchido a partir deste dossiê.

---

## 14. FONTES

| Fonte | Cobertura |
|---|---|
| Call de onboarding — [Fathom 760724003](https://fathom.video/calls/760724003) | 25/07/2026, 56 min. **Transcrição lida na íntegra** (125 blocos de fala) + resumo automático |
| WhatsApp — privado Jane ↔ Nicolas | **27/05 a 03/08/2026 · 236 mensagens** (2º export). Anexos: [29/07](ANEXOS-WHATSAPP-2026-07-29.md) e [03/08](ANEXOS-WHATSAPP-2026-08-03.md) |
| WhatsApp — grupo Jane × Expansion | **24/07 a 03/08/2026 · 26 mensagens** |
| Áudios do WhatsApp | 7 arquivos `.opus` (30/07 a 03/08) — **transcritos** com Whisper `medium` local (sherpa-onnx). Íntegra no [anexo de 03/08](ANEXOS-WHATSAPP-2026-08-03.md#d--transcrição-dos-áudios) |
| Google Drive — pasta da Jane | Doc de onboarding lido na íntegra; estrutura de pastas mapeada |
| Repositório | `OPERACAO-REAL-EXPANSION-2026-07-26.md`, `POP-SOCIAL-MEDIA-v0.1.md` |

**Ressalva de transcrição.** A transcrição do Fathom tem erros de reconhecimento de fala e trechos onde a legenda vira inglês. Nomes normalizados aqui a partir do contexto e do doc do Drive: *"Mil Black"* → **Amil Black**; *"Agrotus"* → **Agrotools**; *"INS"/"NRS"* → **ANS**; *"RN1"* → **NR-1**; *"CPI"* → **UTI**; *"Trasorb"* → **Atrasorb**; *"Dilemo"* → **Lemo**; *"Cauã"* → **Kauã**. Onde a normalização não foi segura, está marcado com ⚠️.

**Lacunas conhecidas** — o que **não** está coberto e só a Jane ou o Nicolas resolvem:
- Os áudios de **27/07 e 29/07** — nenhum dos dois exports os trouxe. São ~10 mensagens, incluindo a conversa sobre acessos (27/07) e a ligação de 6 minutos de 29/07
- Se o **convite de Collab da Agrotools** foi aceito
- **Lista de entregáveis do plano teste** — o valor e o ciclo estão fechados (§9), o conteúdo do pacote não
- O @ atual do Instagram e o número de seguidores
- Se o erro de nome de empresa no primeiro Reels foi corrigido
- Se houve um segundo pagamento em agosto (§11)

**Confirmado por Nicolas:** valor do plano teste — R$2.000 (29/07) · ciclo 24/07 a
24/08 com reunião de alinhamento (03/08) · decisão sobre tráfego pago **segue em
aberto**, não foi resolvida nos áudios (29/07).


---

## 15. FECHAMENTO DO CICLO 1 — 25/07 a 24/08

### O placar

| | Planejado | Entregue |
|---|---|---|
| Posts | 12 | **4** |
| Dias sem publicar | — | **10** (15 a 24/08) |
| Diárias de gravação | 2 | 3 *(29, 30 e 31/07)* |
| Roteiros gravados | 5 | 5 |
| Roteiros 06–14 gravados | 9 | **0** |
| Reunião de fechamento | 24/08 | **não marcada** |

**Não faltou material.** Faltou passar o material pela linha de produção.

### A cadeia de causa

1. **A ligação de alinhamento nunca aconteceu.** Ela pediu 15 minutos em 04/08, foi
   remarcada três vezes e sumiu. A estratégia e o cronograma nunca foram aprovados de
   viva-voz — ficaram no PDF.
2. **Nove reclamações de qualidade em 14 dias**, seis delas de áudio. Cada uma virou
   uma rodada de reedição, e cada rodada comeu um slot do cronograma.
3. **A aprovação travou 5 dias** (13 a 18/08) por permissão de pasta do Drive e portal.
   Foi aqui que a cadência morreu.
4. **20/08 — saída do social media e do tráfego**, anunciada dentro do grupo do cliente.
5. **22/08 — a cliente cobra**, e cobra com a frase do mês promocional.

### O que isso significa

O risco não é mais teórico. O gatilho declarado por ela na call era *"não transmitir o
que eu espero que transmita"* — e o que aconteceu foi pior que isso: **o perfil parou.**

Mas a leitura não é toda ruim, e isso importa para a conversa de renovação:

- Ela **aprovou o plano** — *"ótimo"*, *"ficou top"*
- Ela **aprovou posts** — *"Ah gostei"* (07/08), *"gostei"* (13/08)
- Ela **continua engajada**: revisa, pede ajuste, manda áudio, responde no mesmo dia.
  Cliente que desistiu não revisa vídeo às 21h22
- Ela **viu o reposicionamento do perfil** e respondeu *"Cheguei sim"*

Ela não saiu. Ela está pedindo para ser levada a sério.

### O que muda no ciclo 2 — as três travas

| Trava | Por quê |
|---|---|
| **Banco aprovado com antecedência.** Nunca publicar de material aprovado no mesmo dia | Foi a rodada de revisão que comeu o cronograma |
| **Conferência de áudio antes de subir** — nível de música contra voz, ruído de fundo, áudio até o fim | 6 das 9 reclamações |
| **Zero fricção de acesso.** Toda pasta e todo link de aprovação em "qualquer pessoa com o link", conferido antes de pedir aprovação | Custou 5 dias e o *"é chato"* |

### 🔐 Terceira ocorrência de dado sensível no grupo

Senha do Facebook (27/07) → CPF de terceiro (29/07) → CPF/CNPJ da cliente (14/08).
**Nenhum foi versionado aqui.** O padrão é o canal: documento e credencial não trafegam
em grupo de WhatsApp. Ver §12.
